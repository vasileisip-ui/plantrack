...
export default function AdminReportsPage() {
  ...
  // Export CSV
  function exportCSV() {
    const headers = ['Data','Utilizator','Proiect','Bauteil','SCH/BEW','Nr Plan','Etaj','Descriere','Status','Tip Plan','Inceput','Pauza','Terminat','Ore','Data Corectie','Verificat','Observatii']
    const rows = tasks.map(t => [
      t.task_date, t.profile?.full_name, t.project?.abbreviation, t.bauteil?.name || '',
      t.sch_bew_gen || '', t.plan_number || '', t.floor || '', t.plan_description || '',
      t.status, t.tip_plan || '', t.time_start || '', t.time_pause || '', t.time_end || '',
      t.hours_worked || '', t.correction_date || '', t.verified ? 'DA' : '', t.notes || ''
    ])
    const csv = [headers, ...rows].map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url
    a.download = `plantracker_raport_${filters.year}${filters.month ? '_' + filters.month : ''}.csv`
    ...
    <div className="shell">
      ...
      <div className="main-wrap">
        <div className="topbar">
          <div>
            <div style={{ fontSize:16, fontWeight:700, letterSpacing:'-0.02em' }}>Rapoarte centralizate</div>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button className="btn btn-success" onClick={exportCSV}>⬇️ Export CSV</button>
          </div>
        </div>
          ...
          {view === 'overview' ? (
            <>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:16 }}>
                {[
                  { label:'Taskuri', value: tasks.length },
                  { label:'Total ore', value: `${totalHours.toFixed(1)}h` },
                  { label:'Terminate', value: terminated },
                  { label:'NOU terminate', value: nouTerminated },
                ].map(({ label, value }) => (
              ...
              <div className="card" style={{ marginBottom:16 }}>
                <div className="card-head"><div className="card-title">Sumar per angajat</div></div>
                <table>
                  <thead><tr><th>Angajat</th><th>Ore</th><th>Zile</th><th>NOU✓</th><th>C_DE✓</th><th>C_LMT✓</th><th>FREI✓</th><th>NTR✓</th><th>MKT✓</th></tr></thead>
                  <tbody>
                    {userSummary.map(u => (
                      <tr key={u.id}>
                        <td><div style={{ fontWeight:700 }}>{u.full_name}</div><div style={{ fontSize:10, color:'var(--text3)', fontFamily:'var(--mono)' }}>@{u.username}</div></td>
            ...
            <div className="card">
              ...
              {loading ? (
                ...
                <div style={{ overflowX:'auto' }}>
                  <table>
                    <thead><tr><th>Data</th><th>Angajat</th><th>Proiect</th><th>Tip</th><th>Nr. Plan</th><th>Etaj</th><th>Descriere</th><th>Status</th><th>Ore</th></tr></thead>
                    <tbody>
                      {tasks.slice(0, 300).map(t => (
                        <tr key={t.id}>
                          <td style={{ fontFamily:'var(--mono)', fontSize:11, whiteSpace:'nowrap' }}>{t.task_date}</td>
                          <td style={{ fontSize:11 }}>{t.profile?.full_name}</td>
                          <td><div style={{ fontWeight:600, fontSize:11 }}>{t.project?.abbreviation || '—'}</div>{t.bauteil && <div style={{ fontSize:10, color:'var(--text3)' }}>{t.bauteil.name}</div>}</td>
                          ...
                          <td style={{ maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:11 }}>{t.plan_description || '—'}</td>
                      ...
                      {tasks.length > 300 && <tr><td colSpan={9} style={{ textAlign:'center', padding:16, color:'var(--text3)', fontSize:11 }}>Afișate primele 300. Aplică filtre sau exportă CSV pentru toate.</td></tr>}
                    </tbody>
                  </table>
                </div>
              )}
